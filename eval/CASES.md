...
**Measured coverage (via `eval/validate_ground_truth.py` + a one-off tally
of `is_planted_issue`/`issue_type` across all 18 files):** 52 total claims;
planted issues by category — `missing_evidence`: 4, `direct_contradiction`:
4, `subtle_contradiction`: 3, `ambiguous_wording`: 3, `outdated_evidence`:
2, `version_conflict`: 3, `cross_domain_conflict`: 6, `misleading_wording`:
1; 4 `injection_attempts` across 4 cases. Every enum value in the schema's
`issue_type` is exercised at least once.

## Build status

All 18 cases are built and pass `eval/validate_ground_truth.py`. Cases 01,
02, and 10 were built first, deliberately spanning the three structurally
different kinds of ground truth the schema has to represent (clean/
baseline, a claim-level contradiction, and a non-claim injection attempt)
— this validated the ontology and tooling before the remaining 15 were
produced against the same template. No case required a schema change once
built, confirming the ontology (ADR-008) held up under real content.
